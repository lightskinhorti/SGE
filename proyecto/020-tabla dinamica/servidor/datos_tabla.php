<?php
	if(!isset($_GET['tabla'])){   //si no pones nada coge la tabla clientes
		$tabla = "clientes";
	}else{						//si pones algo, cogeme ese algo
		$tabla = $_GET['tabla'];
	}
	mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);			// Establezco el nivel de retorno de errores de PHP
	$mysqli = mysqli_connect("localhost", "horti", "horti", "proyecto1sge");		// Me conecto a la base de datos
	$query = "
		SELECT * FROM ".$tabla .";
